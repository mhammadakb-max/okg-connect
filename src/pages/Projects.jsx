...
export default function Projects() {
  ...
  const visibleProjects = useMemo(() => {
    const publicProjects = projects.filter((project) => project.status !== 'coming_soon');
    if (!publicProjects.length) return [];
    if (activeFilter === 'All Projects') return publicProjects;
    if (activeFilter === 'Active Projects') return publicProjects.filter((p) => ['in_progress', 'mobilised', 'mobilising', 'awarded'].includes(p.status));
    if (activeFilter === 'Completed Projects') return publicProjects.filter((p) => ['completed', 'closed'].includes(p.status));
    return publicProjects.filter((p) => `${p.scope || ''} ${p.project_type || ''}`.toLowerCase().includes(activeFilter.toLowerCase().replace(' works', '').replace(' deployment', '')));
  ...
  return (
    <>
      ...
      <PageHero title="Projects" intro="Browse verified public project information shared by OKG." eyebrow="Projects" breadcrumb="Projects" />
      <section className="bg-white py-16 md:py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          {visibleProjects.length ? <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{visibleProjects.map((project) => <ProjectCard key={project.id} project={project} />)}</div> : <div className="rounded-2xl border border-border bg-secondary p-8 text-center text-muted-foreground">No public projects are available right now.</div>}
        </div>
      </section>
